...
class ProfitAndLossStatement extends StatefulWidget {
  const ProfitAndLossStatement({Key? key}) : super(key: key);
...
class _ProfitAndLossStatementState extends State<ProfitAndLossStatement> {
 ...
 bool sReturn = false,
  ...
  List<SalesType> salesReturnTypeDataList = [];
  List<TypeItem> dropdownItemsType = [
    TypeItem(1, 'Summary'),
    TypeItem(2, 'ItemWise'),
    TypeItem(3, 'Simple'),
    TypeItem(4, 'Replacement'),
    TypeItem(5, 'Monthly'),
    TypeItem(6, 'Item Analysis'),
    TypeItem(7, 'Product Wise Grouping'),
    TypeItem(8, 'Category Margin'),
    TypeItem(9, 'Daily Wise Margin'),
    TypeItem(10, 'Van Wise Custom Profit'),
  ];
  int valueType = 1;
  ...
  String stockValuationType = 'Prate';
  String sRateType = 'Actual Rate';
  ...
  void initState() {
    ...
    salesReturnTypeDataList = salesReturnTypeList;
  ...
  void itemChangeReturn(bool val, int index) {
    setState(() {
      salesReturnTypeList[index].stock = val;
    });
  ...
  List<Widget> _getChildrenReturn(data) {
    return List<Widget>.generate(
        data.length,
        (index) => CheckboxListTile(
          activeColor: kPrimaryColor,
    ...
    return Scaffold(
      backgroundColor: bagroundColor,
        appBar: AppBar(
                ...
                setState(() {
                  ...
                  if (menuId == 1) {
                    Future.delayed(const Duration(milliseconds: 1000), () {
                      _createPDF(
                              'P&L Statement' + ' Date :' + fromDate! + ' - ' + toDate!)
                          .then((value) =>
          ...
          title: const Text('Profit and Loss Statement'),
        ),
  ...
  reportView(title) {
    ...
    if (isType) {
     ...
     var statementType = title == 'Summery'
        ? 'P_l_Summery'
        : title == 'ItemWise'
            ? 'P_l_ItemWise_New'
            : title == 'Simple'
                ...
                : title == 'Replacement'
                    ...
                    : title == 'Monthly'
                        ? 'P_l_Monthly'
                        : title == 'Item Analysis'
                            ? 'ItemWise_Profit_Analysis'
                            : title == 'Product Wise Grouping'
                                ? 'Simple_ItemWise_p&l_stamnt'
                                : title == 'Category Margin'
                                    ? 'P_L_Category_Margin'
                                    : title == 'Daily Wise Margin'
                                      ? 'P_l_DayWise_Margin'
                                      : title == 'Van Wise Custom Profit'
                                        ? 'Van Wise Custom Profit'
                                        : 'P_l_Summery';
    ...
    for (var data in salesReturnTypeList) {
      if (data.stock) dataSReturnType.add({'id': data.id});
    }
    ...
    if (newMode) {
      ...
      var dataJson = '[' +
          json.encode({
            ...
            'salesReturnType': dataSReturnType != null
                ? jsonEncode(dataSReturnType)
                : jsonEncode([
                    {'id': 0}
                  ]),
            'toName': '',
            ...
            'cashSale': '0',
            'creditSales': '0',
            'rateType': stockValuationType ,//stockValuation ? 'RPRate' : 'PRate', 
            'sRateType' : sRateType,
            'serialNo': '',
            'supplierId': supplier != null ? supplier.id : '0',
            'srCheck': sReturn ? '1' : '0',
            'serviceCheck': serviceEntry ? '1' : '0',
            'expenseCheck': expense ? '1' : '0',
            'counterNo': '',
            'cardNo': '',
            'salesman2': '0',
            'typeOfSupply': '',
            ...
            'hsn': '',
            'supLed': '0',
            'jobcardCheck': jobcardEntry ? '1' : '0',
          }) +
          ']';
    debugPrint(dataJson);
      return FutureBuilder<List<dynamic>>(  
        future: api.getProfitAndLossReport(dataJson),
          ...
          child: Column(
                  ...
                  child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(
                            value: 'RealPrate',
                            child: Text('RealPrate'),
                          ),
                        ],
                        onChanged: (value) {
                          if (value == null) return;
                          setState(() {
                            stockValuationType = value;
                          });
                        },
                      ),
                    ),
                  ...
                  child: DropdownButtonHideUnderline(
                      child: DropdownButton<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                            value: 'Actual Rate',
                            child: Text('Actual Rate'),
                          ),
                          DropdownMenuItem(
                            value: 'RealRate',
                            child: Text('Real Rate'),
                          ),
                        ],
                        onChanged: (value) {
                          if (value == null) return;
            ...
            //         ),
             Container(
                ...
                child: ExpansionTile(
                  ...
                  children: [
                    _buildCheckItem(
                      title: 'Sales Return',
                      value: sReturn,
                      onChanged: (v) => setState(() => sReturn = v),
                    ),
                    _buildCheckItem(
                      title: 'Service Entry',
                      value: serviceEntry,
                      onChanged: (v) => setState(() => serviceEntry = v),
                    ),
                    _buildCheckItem(
                      title: 'Jobcard Entry',
                      value: jobcardEntry,
                      onChanged: (v) => setState(() => jobcardEntry = v),
                    ),
                    _buildCheckItem(
                      title: 'Expense',
                      value: expense,
                      onChanged: (v) => setState(() => expense = v),
                    ),
                    _buildCheckItem(
                      title: 'Branch Transfer',
                      value: branchTransfer,
                      onChanged: (v) => setState(() => branchTransfer = v),
                    ),
                  ],
                ),
              ),
              ...
              salesReturnTypeList.isNotEmpty
                  ? Container(
                      width: MediaQuery.of(context).size.width,
                      decoration: BoxDecoration(
                          border: Border.all(color: grey),
                      ...
                      child: ExpansionTile(
                        ...
                        children: _getChildrenReturn(salesReturnTypeList),
                      ),
                    )
                  : Container(),
              const SizedBox(
