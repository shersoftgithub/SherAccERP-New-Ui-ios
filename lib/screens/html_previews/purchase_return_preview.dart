  ...
  previewWidget() {
    var taxInvoice = dataInformation != null
        ? dataInformation['TaxType'] == 'T'
            ...
            : false
        : false;
    var invoiceHead = ComSettings.appSettings(
        'String', 'key-purchase-return-head', 'PURCHASE RETURN');
    // companyTaxMode == 'INDIA' ? '' : '';
...
void printSunmiV1(dataAll) async {
  ...
  int printCopy =
      ComSettings.appSettings('int', 'key-dropdown-print-copy-view', 0);
  int printerModel =
      ComSettings.appSettings('int', 'key-dropdown-printer-model-view', 0);
  ...
  if (result!) {
    ...
    if (taxSale) {
     ...
     await SunmiPrinter.printRow(cols: [
      ...
      await SunmiPrinter.printRow(
        cols: [
          ColumnMaker(
              text: "Description", width: 16, align: SunmiPrintAlign.CENTER),
          ColumnMaker(text: "Qty", width: 5, align: SunmiPrintAlign.CENTER),
          ColumnMaker(text: "Price", width: 8, align: SunmiPrintAlign.CENTER),
          ColumnMaker(text: "Total", width: 8, align: SunmiPrintAlign.CENTER)
        ],
      );
      await SunmiPrinter.setCustomFontSize(20);
      await SunmiPrinter.lineWrap(1);
    ...
    await SunmiPrinter.printRow(
      cols: [
        ColumnMaker(
            text: "Net Amount :", width: 16, align: SunmiPrintAlign.LEFT),
        ColumnMaker(
            text: double.tryParse(inf['GrandTotal'].toString())!
                .toStringAsFixed(2),
            width: 16,
            align: SunmiPrintAlign.RIGHT)
      ],
...
//
    ...
    // if (isQrCodeKSA) {
    //   if (taxSale) {
    //     await poSunmiPrinter.printQRCode(SaudiConversion.getBase64(
    //         settings.name,
    //         ComSettings.getValue('GST-NO', settings),
